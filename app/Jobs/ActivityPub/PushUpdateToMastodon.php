...
class PushUpdateToMastodon implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(): void
    {
        try {
            ...
            Log::error('PushUpdateToMastodon: Failed to fetch post', ['postId' => $this->postId, 'error' => $e->getMessage()]);

            return;
        }

        if ($postDto->visibility !== Visibility::PUBLIC) {
            Log::info('PushUpdateToMastodon: Skipping non-public post', ['postId' => $this->postId]);
        ...
        $note->updated = $postDto->updatedAt;
        $updateActivity = new UpdateHydrator()->hydrate($actorUrl, $note, true)->toArray();
        ...
        foreach ($followers as $follow) {
            ...
            DeliverActivityPubActivity::dispatch($postDto->user, $follow->follower_actor_id, $inbox, $updateActivity);
        }
    }
}
